...
#### 왜 자체 냉장고(Harbor)를 쓸까?

Docker Hub 같은 공용 냉장고를 쓸 수도 있는데, 왜 굳이 자체 냉장고를 운영할까요? 핵심 이유는 **비밀 양념(시크릿)의 안전한 관리**입니다.

일반적인 Kubernetes 배포에서는 API 키, DB 비밀번호 같은 비밀값을 Kubernetes Secret이라는 별도의 금고에 넣어두고, 도시락(컨테이너)이 서빙될 때 금고에서 꺼내 쓰는 방식을 사용합니다. 하지만 이 방식은 식당(K3S) 내부에 금고를 만들고 관리해야 하므로, 식당 접근 권한이 필요하고 관리 부담이 생깁니다.

GrowWeek은 다른 접근을 택했습니다. **도시락을 만들 때(Docker 빌드 시점) 비밀 양념을 미리 넣어서 밀봉**합니다.

```mermaid
flowchart LR
    GHS["GitHub Secrets\n(비밀 양념 금고)"] --> GA["주방 로봇\n(GitHub Actions)"]
    GA -- "빌드 시 양념 주입" --> IMG["완성된 도시락\n(Docker 이미지)"]
    IMG --> Harbor["자체 냉장고\n(Harbor)"]

    style GHS fill:#333,color:#fff
    style Harbor fill:#333,color:#fff
```

이 방식의 장점을 식당 비유로 설명하면 이렇습니다.

| 항목 | 일반 방식 (Kubernetes Secret) | GrowWeek 방식 (빌드 시점 주입) |
|------|---------------------------|--------------------------|
| 비밀 양념 보관 | 식당 안에 금고를 만들어 보관 | 주방에서 도시락에 미리 넣어 밀봉 |
| 식당 접근 필요 | 금고 관리를 위해 식당에 자주 출입 | 식당 출입 최소화 (도시락만 전달하면 됨) |
| 양념 변경 시 | 식당 금고를 직접 열어 교체 | 새 도시락을 만들어 냉장고에 넣으면 끝 |

이렇게 하면 **식당(K3S 클러스터) 접근을 최소화**할 수 있습니다. 비밀값을 위해 Kubernetes Secret을 만들거나 관리할 필요가 없고, 주방 로봇(GitHub Actions)이 GitHub Secrets에서 꺼낸 값을 빌드 시점에 주입하고, 완성된 도시락은 자체 냉장고(Harbor)에 안전하게 보관됩니다.

자체 냉장고를 쓰는 이유가 바로 여기에 있습니다. 비밀 양념이 들어간 도시락은 아무나 접근할 수 있는 공용 냉장고(Docker Hub)에 두면 안 되니까요.
